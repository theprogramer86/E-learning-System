...
if(isset($_POST['save']))
{
    $gurukul = $conn->query("SELECT * FROM gurukkal_master where sno = '".$_POST['gurukul']."' ")->fetch_assoc();
    // print_r($gurukul);die();
    $cart =  json_decode($_COOKIE["Cart"], true);
    ...
    if($itemsquantity != 0)
    {
        ...
        foreach($cart as $data)
        {
            ...
            $sql = "INSERT INTO sales (sno, billno, dat, iname,quan, price, amount, total, pby, remarks, user, last, company, gname, per) 
            VALUES('".$last."','".$bill."','".date("Y-m-d")."','".$value['iname']."','".$value['qty']."','".$value['price']."','".($value['price']*$value['qty'])."','".$total."','".$_POST['pby']."','".$_POST['remark']."','".$user."','".date("d-m-Y h:i:s A")."','".$company."','".$gurukul['gname']."','".$gurukul['per']."' )";
                   ...
                   <table id="carttotals" style="width: 100%;background-color:#4E73DF;color: white; font-weight: 700;font-size: 20px">
                    ...
                    <div class="row" style="margin-top: 10px">
                      ...
                      <div class="col-lg-6" style="margin-bottom:2px">
                         ...
                         <select class="form-control form-control-user" placeholder="Type here" name="gurukul">
                        <option value="0">Select Gurukkal</option>
                         <?php
                         $i_sqlddd = "SELECT * FROM gurukkal_master where company = '".$company."'";
                            $i_resultdd = $conn->query($i_sqlddd);
                            while($row = $i_resultdd->fetch_assoc()) {
                                // array_push($items,$row);
                                echo '<option value="'.$row['sno'].'">'.$row['gname'].'</option>';
                            }
                         ?>
                         </select>
                         <br>
...
function addToCart(name, price) {
    ...
    if (typeof(Storage) !== "undefined") {
      ...
      if(price <= 0)
        pprice= prompt("Please Enter Price", price);
      
      let data = {'iname': name, 'price': pprice, 'qty': 1};
